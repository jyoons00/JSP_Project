    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
            ...
            $(data.comments).each(function(index, comments) {
                ...
                $("#cmt-ul" + revId).append(`
                    <li class="comment-content" data-comment-id ="${comments.cmt_id}" data-rev-id ="${comments.rev_id}">
	 ...
	 $.ajax({
				  ...
				  let deleteButton = '';
	                ...
	                if (comments.user_id === userId) {
            		  ...
            		  $("#cmt-ul" + revId).append(`<li class="comment-content" data-comment-id ="${comments.cmt_id}" data-rev-id="${comments.rev_id}" >
                                                        <div class="comment-author">
                                                            ${comments.user_name}
                                                        </div>
                                                        <div class="comment-msg">
...
$(document).on("click", ".comment_v2__edit_link.js-comment-edit-dropdown-link", function(event) {
    ...
    // 클릭된 삭제 버튼의 가장 가까운 부모 요소 중 data-comment-id 속성을 가진 요소를 찾음
    ...
    // 여기에 AJAX 요청을 추가하여 서버에서 해당 댓글을 삭제할 수 있음
    // 예시:
    if (confirm("댓글을 삭제하시겠습니까?")) {
        $.ajax({
            url: "/projectOhora/review/deleteComment.ajax",
            type: "POST",
            dataType: "json",
            data: { cmt_id: commentId
						, revId : revId },
            success: function(response) {
				console.log(response)
                if (response.rev_id) {
					baroComment2(response.rev_id); 
